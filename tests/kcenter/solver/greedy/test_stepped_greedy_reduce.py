...
def test_generator_greedy_reduce_basic_graph_outlier_colourful_clustering():
    ...
    assert next(solution) == ({0: {0, 1, 2, 3, 4}}, set(), pytest.approx(6.113, FLOAT_ERROR_MARGIN),
                              "The initial center is arbitrarily chosen, its coordinates are (1.3, 2.6). It is a blue point.", True)
    assert next(solution) == ({0: {0, 1, 2}, 4: {3, 4}}, set(), pytest.approx(5.515, FLOAT_ERROR_MARGIN),
                              "We find the point which has the maximum distance from its closest center, which is the point at (6.4, 4.7). It is a red point 5.515 distance away. This makes the current cost 5.515.", True)
    assert next(solution) == ({0: {0, 1, 2}, 4: {3, 4}}, set(), pytest.approx(3.785, FLOAT_ERROR_MARGIN),
                              "Our 2 centers have been chosen. The current cost is 3.785, we will continue to reduce the cost until the solution does not meet the constraints.", True)
    assert next(solution) == ({0: {0, 1, 2}, 4: {3, 4}}, set(), pytest.approx(0.707, FLOAT_ERROR_MARGIN),
                              "decrease weight to 0.707", True)
    assert next(solution) == ({0: {0, 1, 2}, 4: {3, 4}}, set(), pytest.approx(0.707, FLOAT_ERROR_MARGIN),
                              "completed reduced solution to radius of 0.707", False)
