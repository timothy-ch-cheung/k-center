...
def test_step_through_basic_graph(basic_graph):
    ...
    response = test_client.post("/api/v1/step/next", json={"id": id})
    ...
    response = test_client.post("/api/v1/step/next", json={"id": id})
    assert response.status_code == 404
    assert response.get_json() == {"message": f"{id} is not an active problem instance"}
...
def test_step_through_basic_graph_pbs_inspect(basic_graph_pbs, seed_random):
    ...
    response = test_client.post("/api/v1/step/inspect", json={"id": id})
    assert response.get_json()["step"] == {
        "active": True,
        "inspect": True,
...
def test_inspect_terminate_generator(basic_graph_pbs, seed_random):
    ...
    test_client.post("/api/v1/step/next", json={"id": id})
    test_client.post("/api/v1/step/next", json={"id": id})
    test_client.post("/api/v1/step/next", json={"id": id})
    response = test_client.post("/api/v1/step/next", json={"id": id})
